...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        // If the friendship request doesn't exist, create a new one
        if (!existingRequest) {
          // Update the existing friendship request to have the status 'accepted'
  ...
  decline: procedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       */
      // Update the existing friendship request status to 'declined'
